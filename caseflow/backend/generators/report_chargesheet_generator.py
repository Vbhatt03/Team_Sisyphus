...
def generate_final_report_for_ai(case_diary_text: str, statement_narrative: str, victim_medical_opinion: str, accused_medical_opinion: str) -> str:
    ...
    report += (case_diary_text or "[Case diary not available]") + "\n\n"
    report += "II. FULL VICTIM STATEMENT\n--------------------------\n"
    report += (statement_narrative or "[Statement narrative not available]") + "\n\n"
    report += "III. MEDICAL OPINIONS\n--------------------------\n"
    report += f"Victim Medical Opinion: {victim_medical_opinion or '[N/A]'}\n"
    report += f"Accused Medical Opinion: {accused_medical_opinion or '[N/A]'}\n\n"
    report += "==================================================================\nEnd of Report\n"
    return report

# ---------------------------------------------------------------------------
# Compatibility wrapper for CaseFlow adapter
# The adapter (adapt_chargesheet_generator) expects a function named
# `generate_final_report(fir_data, statement_data, victim_medical_data, accused_medical_data, case_diary_text)`.
# Originally this script exposed only `generate_final_report_for_ai` with a
# different argument signature. This wrapper keeps the adapter working
# without changing the adapter code.
# ---------------------------------------------------------------------------
def generate_final_report(
    ...
    """Generate the consolidated final report text expected by the adapter.

    Parameters mirror what the CaseFlow adapter passes. We derive the
    specific narrative/opinion strings and delegate to
    `generate_final_report_for_ai` to produce the body.
    """
    statement_narrative = None
    if isinstance(statement_data, dict):
        statement_narrative = statement_data.get('narrative') or statement_data.get('statement')

    victim_med_opinion = None
    if isinstance(victim_medical_data, dict):
        victim_med_opinion = (
            victim_medical_data.get('provisional_medical_opinion') or
            victim_medical_data.get('opinion') or
            victim_medical_data.get('findings')
        )

    accused_med_opinion = None
    if isinstance(accused_medical_data, dict):
        accused_med_opinion = (
            accused_medical_data.get('opinion') or
            accused_medical_data.get('provisional_medical_opinion') or
            accused_medical_data.get('findings')
        )

    return generate_final_report_for_ai(
        case_diary_text=case_diary_text or "",
        statement_narrative=statement_narrative or "",
        victim_medical_opinion=victim_med_opinion or "",
        accused_medical_opinion=accused_med_opinion or ""
    )
